...
- 대화 세션은 열 때마다 응답 횟수 상한이 3~5회로 고정되며, 상한 도달 시 자동 종료된다.
- 대화 세션은 닫힌 후 1분 쿨다운이 지나야 다시 열 수 있다.
...
- `API 키 설정` 버튼은 `대화하기`를 눌러 대화 영역이 열린 상태에서만 표시되며, `대화하기`를 다시 누르면 대화 영역과 키 설정 패널이 함께 접힌다.
...
- 대화 세션이 3~5회 응답 후 자동 종료되는지, 종료 후 1분 전에는 재오픈이 차단되는지 확인
