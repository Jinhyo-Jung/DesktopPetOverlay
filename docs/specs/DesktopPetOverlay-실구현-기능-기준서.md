...
- 앱 시작 시 렌더러 모듈 로딩 전에도 로컬 저장 EXP로 성장 단계를 계산해 초기 얼굴/스테이지/임시 캐릭터 프리뷰에 즉시 반영한다.
- 스프라이트 JSON(`source/pet_sprites/main_cat.json`) 기반으로 상태별 프레임(`idle/walk/jump/fall/drag`) 재생이 된다.
- 단계 폴더에서 변형 이미지(`*_01`, `*_02`, ...)가 누락되면 같은 단계의 기본 감정 이미지(`happy`, `neutral`, `sleep`, `sick`, `dirty`)로 자동 대체된다.
...
- 패널 최상단에 `대화하기` 기능이 있고, 세션 단위로 펫과 텍스트 대화를 진행할 수 있다.
- 대화 세션의 응답 횟수 상한은 5회이며, 5회 소진 시 자동 종료된다.
- 사용자가 중간에 대화창을 닫은 경우에는 재오픈 쿨다운이 적용되지 않고, 5회 소진으로 세션이 종료된 경우에만 1분 쿨다운이 적용된다.
- `?` 버튼 오른쪽의 `R` 버튼은 미열람 일일 리포트가 있을 때만 표시되고, 1회 열람 후 숨겨진다.
- 패널 최하단 `종료` 버튼으로 앱 프로세스를 종료할 수 있다.
...
- 성장 단계가 상승(`Egg->Baby`, `Baby->Teen`, `Teen->Adult`)하면 단계별 고정 이벤트 메시지를 1회 표시한다.
...
- 앱 종료 시점의 일일 요약/격려 리포트를 로컬스토리지에 저장하며, 종료 직전에 즉시 열람 여부를 confirm으로 묻는다.
- 대화창 재오픈 쿨다운 상태(`lastOpenedAt`)를 로컬스토리지에 저장해 재실행 후에도 1분 제한을 유지한다.
- 앱 종료 경로(`종료 버튼`, `Alt+F4`, 창 닫기) 모두 동일한 종료 훅으로 진입해 리포트 저장/열람 확인 로직이 동일하게 적용된다.

### 6-1) OpenAI 연동 대화
- OpenAI 호출은 렌더러 직접 호출이 아니라 `preload` 브리지를 통해 `main` IPC(`pet-chat:send`)로 수행한다.
- API 키는 코드 하드코딩 없이 앱 내 설정으로만 읽는다.
- 앱 내 설정은 `userData/openai-config.json`에 저장되며, UI에서 키를 등록/삭제할 수 있다.
- `대화하기`를 누르면 상단에 대화 설정 톱니 버튼이 표시되고, 여기서 저장 폴더 열기/키 삭제/키 재입력을 수행할 수 있다.
- OpenAI 오류(예: 키 불일치, 쿼터 초과)는 원문 코드 대신 사용자 친화 문구로 표시된다.
...
- 단계 전환 시 고정 이벤트 메시지가 1회 표시되는지 확인
- 앱 종료 시 일일 리포트 확인 팝업이 표시되고, 미열람으로 종료하면 다음 실행에서 `R` 버튼으로 열람 가능한지 확인
- 대화 세션이 5회 응답 후 자동 종료되는지, 5회 소진 종료 직후 1분 동안 재오픈이 차단되는지 확인(중간 수동 닫기 시 즉시 재오픈 가능)
- `종료` 버튼, `Alt+F4`, 창 닫기 각각에서 동일하게 종료되고 리포트 로직이 동작하는지 확인
